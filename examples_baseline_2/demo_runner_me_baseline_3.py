...
# ImageFile.LOAD_TRUNCATED_IMAGES = True
    ...
    def getinteraction_intensity(self,imgd,cam,sd_len,Z):
        [p,q]=imgd.shape
        imgd=np.double(imgd)
        # print(imgd)
        # imgd -> black and white image
        bord=10
        KK=cam
        px=KK[0,0]
        py=KK[1,1]
        ...
        Gx_arr=np.double(np.zeros((p,q)))
        Gy_arr=np.double(np.zeros((p,q)))
        m = 0
        Lsd=np.double(np.zeros((sd_len,6)))
        xy_arr=np.double(np.zeros((sd_len,4)))
        for i in range(bord+1,p-bord):
            for j in range(bord+1,q-bord):
    ...
    def getfeature_intensity(self,img):
        bord=10
        img=np.double(img)

        [a,b]=img.shape
        s=np.zeros(((a-2*bord)*(b-2*bord),1))
        # print(s.shape)
        arr=np.zeros(((a-2*bord)*(b-2*bord),3))
        k=0
        for i in range(bord+1,a-bord):
            for j in range(bord+1,b-bord):
                # print(k)
                # print(i,j)
                s[k,0]=img[i,j]
                arr[k,0]=i
    ...
    def example(self,Vx,Vy,Vz,Wx,Wy,Wz,harish):
        ...
        # print(vik)
                                                            ### Change here :: 
        total_frames=19
        harish="/home/yvsharish/working/habitat-sim/image_baseline_2/test.rgba." + str(total_frames).zfill(5) +".png"
        imgd=Image.open(harish)
        imgd=np.array(imgd)
        imgd=self.rgb2gray(imgd)
        # print(imgd.shape)

        sId=self.getfeature_intensity(imgd)
        [sd_len,p_i]=sId.shape
        ...
        while photo_error>500:
            ...
            print('Before IBVS')
            if(frames==0):
                harish="/home/yvsharish/working/habitat-sim/image_baseline_2/test.rgba." + str(frames).zfill(5) +".png"
            else:
                # harish="/home/harish/RRC/ICRA_2019/habitat/habitat-sim/image/test.rgba." + str(total_frames).zfill(5) +".png"
                harish="/scratch/yvsharish/working/habitat-sim/image_baseline_2_output/test.rgba." + str(frames).zfill(5) +".png"
            ...
            nx, ny = (512,384)
            # cam=np.asarray([[64,0,64],[0,64, 64 ],[ 0, 0, 1]])
            cam=np.asarray([[nx/2,0,nx/2],[0,ny/2, ny/2 ],[ 0, 0, 1]])

            cam=np.double(cam)
            sI=self.getfeature_intensity(im)
            error=sI-sId
            desired_depth=0.51

            if(frames==0):
                alpha="/home/yvsharish/working/habitat-sim/image_baseline_2/test.depth." + str(frames).zfill(5) +".png"
            else:
                # harish="/home/harish/RRC/ICRA_2019/habitat/habitat-sim/image/test.rgba." + str(total_frames).zfill(5) +".png"
                aplha="/scratch/yvsharish/working/habitat-sim/image_baseline_2_output/test.depth." + str(frames).zfill(5) +".png"
            im_depth=Image.open(alpha)
            Z=(np.array(im_depth))
            Z=Z.astype('float64')       
            # print(imgd)
            Lsd=self.getinteraction_intensity(im,cam,sd_len,Z)
            # print(Lsd)
            # Lsd = Lsd.astype(float)
            Hsd = np.matmul(np.transpose(Lsd),Lsd)
            # print(Hsd)
            diagHsd = np.diag(np.diag(Hsd))
            # print(diagHsd)
            normeError=np.linalg.norm(error)
            H=np.add(np.multiply(mu,diagHsd) , Hsd)
            H = np.linalg.inv(H)
            # print(H)
            e = np.matmul(np.matmul(H ,np.transpose(Lsd)),error)
            # print(e)
            vc = - np.multiply(uuu,e)
            print(vc)
            ...
            command="python2 homogeneous_baseline_3.py"
            os.system(command)
