...
# ImageFile.LOAD_TRUNCATED_IMAGES = True
    ...
    def example(self,Vx,Vy,Vz,Wx,Wy,Wz,harish):
        ...
        while photo_error>500:
            ...
            if(frames==0):
                ...
                if self._sim_settings["save_png"]:
                    if self._sim_settings["color_sensor"]:
                        ...
                        state=p.get_state()
                        if not self._sim_settings["silent"]:
                            print("Shreya-----------position\t", state.position, "\t", "rotation\t", state.rotation,"imgi",state.rotation.imag, "real",state.rotation.real,"sensorstate",state)
            ...
            print(harish)
            f_vel=open("/scratch/yvsharish/working/aaaaa/baseline_2_velocities_single_1_new.txt","w+")
            mat = []
            j=0
            data1=str(data)
            data1=data1[3:-2]
            ...
            ##################################
            f_vel.write("%0.6f %0.6f %0.6f %0.6f %0.6f %0.6f \n" %(Vx,Vy,Vz,Wx,Wy,Wz))
            f_vel.close()
