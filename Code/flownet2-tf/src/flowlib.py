...
def flow_to_image(flow):
    ...
    """
    print("IN FLOW_TO IMAGE- size of flow",np.array(flow).shape)
    u = flow[:, :, 0]
    v = flow[:, :, 1]
    print("IN FLOW_TO IMAGE- size of u",np.array(u).shape)
    ...
    rad = np.sqrt(u ** 2 + v ** 2)
    avg_flow= np.mean(rad)
    print("IN FLOW_TO IMAGE- size of avgflow",avg_flow)
    
    #write the avg_flow to some file
