...
class Net(object):
    ...
    def test(self, checkpoint, out_path, save_image=True, save_flo=False):
        ...
        input_a = tf.placeholder(shape = [128, 128, 3], dtype = tf.float32, name = "source_image")
        input_b = tf.placeholder(shape = [128, 128, 3], dtype = tf.float32, name = "target_image")
        inputs = {
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                if target.max() > 1.0:
                    target = target / 255.0
                pred_flow = sess.run(pred_flow, feed_dict = {input_a : source,  input_b : target})[0, :, :, :]
                save_flo = True 
